...
namespace G4CEMC
{
  ...
  double bottomradius = 65;  // cm
  double topradius = bottomradius+11.8;  // cm (for 40 layers)
  double negrapidity = 0.01;
  double posrapidity = 1.4;

  //For CORE, we have a crystal calorimeter on negative eta side
  double neglength = 140; // cm
...
double CEmc(PHG4Reco *g4Reco, double radius)
{
  ...
  if (radius > G4CEMC::bottomradius)
  {
    cout << "inconsistency, radius: " << radius
         << " larger than allowed inner radius for CEMC" << endl;
    gSystem->Exit(-1);
  }

  radius = G4CEMC::bottomradius;
  ...
  //Negative eta side
  cemc = new PHG4CylinderSubsystem("CEMC_NEG", 0);
  cemc->set_double_param("radius", G4CEMC::bottomradius);
  cemc->set_string_param("material", "G4_PbWO4");
  cemc->set_double_param("thickness", G4CEMC::topradius-G4CEMC::bottomradius);
  cemc->set_int_param("lightyield", 1);
  cemc->set_int_param("lengthviarapidity", 0);
  cemc->set_double_param("length", G4CEMC::neglength);
  cemc->set_double_param("place_z", -1.*(G4CEMC::neglength/2.)-0.01);

  cemc->SuperDetector("CEMC_NEG");
  cemc->SetActive();
  cemc->OverlapCheck(OverlapCheck);
  g4Reco->registerSubsystem(cemc);
  ...
  double l2 = G4CEMC::neglength + 0.01;
...
void CEMC_Cells()
{
  ...
  double radius = G4CEMC::bottomradius;
